...
async function main() {
    ...
    if (parseFloat(balance) < 0.01 && network !== "hardhat" && network !== "localhost") {
        console.error("\nError: Deployer balance too low - need at least 0.01 tBNB for gas.");
    ...
    console.log("\n" + "=".repeat(60));
    console.log(`  Confirmed FlashGap deployed to: ${contractAddress}`);
    console.log("=".repeat(60));
    ...
    // Post-deploy: setMinProfitBps to emit an event
    if (network !== "hardhat" && network !== "localhost") {
        console.log("Post-deploy config TX (setMinProfitBps)...");
        const tx = await flashGap.setMinProfitBps(MIN_PROFIT_BPS);
        const receipt = await tx.wait();
        console.log(`  Confirmed TX 2: ${receipt.hash}`);
    ...
    if (network !== "hardhat" && network !== "localhost") {
        ...
        try {
            ...
            console.log("Confirmed Contract verified on BscScan!");
        } catch (err) {
            console.log("WarningVerification skipped:", err.message);
        }
    }
